...
public class RangeFinder
{
        ...
        public List<OverlayTile> GetTilesInAtkRange(Vector2Int location, int range)
        {
            var startingTile = MapManager.Instance.map[location];
            var inRangeTiles = new List<OverlayTile>();
            int stepCount = 0;
            ...
            while (stepCount < range)
            {
                ...
                foreach (var item in tilesForPreviousStep)
                {
                    surroundingTiles.AddRange(MapManager.Instance.GetSurroundingAtkTiles(new Vector2Int(item.gridLocation.x, item.gridLocation.y)));
                }
